...
class Line(object):
    ...
    def __init__(self, coord0, coord1):
        ...
        if self.x0 == self.x1:
            ...
            self.xstep = (self.x1-self.x0)//abs(self.x1-self.x0)
            self.ystep = (self.y1-self.y0)//abs(self.y1-self.y0)

    def __str__(self):
        ret = "(" + str(self.x0) + ", " + str(self.y0) + ") to (" + str(self.x1) + ", " + str(self.y1) + ")"
        if hasattr(self, "xstep") and hasattr(self, "ystep"):
            ret += " with xstep " + str(self.xstep) + " and ystep " + str(self.ystep)
        return ret
    ...
    with open("day5_input.txt") as f:
        ...
        for line in lines:
            ...
            if line.orientation == "horizontal":
                ...
                for x,y in zip(range(line.x0, line.x1+line.xstep, line.xstep), range(line.y0, line.y1+line.ystep, line.ystep)):
                    grid[y,x] += 1
            # print(grid)
